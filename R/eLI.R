...
function(nSim=500,visits=3,n=c(560,563,295),x1mean=c(8.8,12.8,16.8),x1sd=c(2.1,2.2,2.9), x2mean=c(-0.09,0.02,0.13),x2sd=c(0.4,0.4,0.4),beta1=c(-0.01),beta2=c(0.06),betaI=c(-0.004,-0.01,-0.02),Sigma=matrix(c(0.1,0.07,0.07,0.07,0.1,0.07,0.07,0.07,0.1),nrow=3,ncol=3,byrow=T),alpha=0.05,plot.pdf=T,plot.label="",plot.name="eLIplot.pdf",seed=1){
  ...
  ######################################
  requireNamespace(nlme) # for lme
  requireNamespace(MASS) # for mvrnorm
  ...
  if(plot.pdf==T){
    pdf(plot.name)
    plot(betaI,results[1,],xlab=expression(beta),ylab=expression("Proportion of Simulations" ~ H[0] ~ "rejected"),main=plot.label,type="b")
    dev.off()	
  }
